...
function checkUsername($username){
	if(!preg_match('/^[1-9][0-9]\d{8}$/',$username)){
		return '用户名格式不符合要求(请输入10位学号或工号)';
	} //'/^1[358]\d{9}$/'
	return true;
}
...
function checkPassword($password){
	if(!preg_match('/^\w{6,16}$/',$password)){
		return '密码格式不符合要求（长度6~16位，只允许英文字母，数字，下划线）';
	}
	return true;
}
function check2Password($password,$password2){
